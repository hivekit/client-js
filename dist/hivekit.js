...
var RealmHandler = class {
  ...
  get(id) {
    ...
    return this._client._sendRequestAndHandleResponse(msg, (response) => {
      const realm = new Realm(id, response[C.FIELD.DATA][C.FIELD.LABEL], response[C.FIELD.DATA][C.FIELD.DATA] || {}, this._client);
      ...
      this.subscribe().then((subscription) => {
        subscription.on("update", (update) => {
          if (update.action === FIELDS.ACTION.UPDATE.FULL && update.realmId === id) {
            this._client._sendRequestAndHandleResponse(msg, (response2) => {
              const newData = response2[C.FIELD.DATA][C.FIELD.DATA] || {};
